...
def bigvgan_base_22khz_80band(progress: bool = True, pretrained: bool = True):
    ...
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            url=URLS["bigvgan_base_22khz_80band"], map_location="cpu", progress=progress
        )
...
def bigvgan_22khz_80band(progress: bool = True, pretrained: bool = True):
    ...
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            url=URLS["bigvgan_22khz_80band"], map_location="cpu", progress=progress
        )
        model.load_state_dict(state_dict)
    model.eval()
    for param in model.parameters():
...
def bigvgan_base_24khz_100band(progress: bool = True, pretrained: bool = True):
    ...
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            url=URLS["bigvgan_base_24khz_100band"],
            map_location="cpu",
            progress=progress,
        )
        model.load_state_dict(state_dict)
...
def bigvgan_24khz_100band(progress: bool = True, pretrained: bool = True):
    ...
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            url=URLS["bigvgan_24khz_100band"], map_location="cpu", progress=progress
        )
        model.load_state_dict(state_dict)
    model.eval()
    for param in model.parameters():
